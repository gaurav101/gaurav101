# 👋 Hello! I'm [Gaurav Kumar](https://gaurav101.github.io/)

Full-stack developer passionate about building scalable applications and exploring new technologies. Welcome to my GitHub profile! 

---

## 🛠️ Tech Stack

| Category | Technologies |
|----------|---------------|
| **Languages** | Java, JavaScript, TypeScript, Python, PHP, Rust, HTML |
| **Frontend** | React, Redux, D3.js, Amplify |
| **Backend** | Spring Boot, Node.js, REST APIs |
| **Tools** | Git, Docker, AWS Amplify |

---

## 📚 Featured Projects

### 🚀 Backend & Java Development
- **[springboot-v1](https://github.com/gaurav101/springboot-v1)** - Learning Spring Boot framework project
- **[Java-practice](https://github.com/gaurav101/Java-practice)** - Java playground for practice and learning
- **[java-practice-basic](https://github.com/gaurav101/java-practice-basic)** - Basic Java practice exercises
- **[observer-java](https://github.com/gaurav101/observer-java)** - Observer design pattern implementation in Java
- **[chat-bot-backend](https://github.com/gaurav101/chat-bot-backend)** - Chat bot backend service

### ⚛️ Frontend & React Projects
- **[LuminaJS](https://github.com/gaurav101/LuminaJS)** - zero deepdency Image processing library
- **[react-redux-learning](https://github.com/gaurav101/react-redux-learning)** - React with Redux state management
- **[amplify-react](https://github.com/gaurav101/amplify-react)** - React app with AWS Amplify integration
- **[react-one](https://github.com/gaurav101/react-one)** - React.js playground project
- **[react-to-do](https://github.com/gaurav101/react-to-do)** - To-do application built with React
- **[Decision-tree-editor](https://github.com/gaurav101/Decision-tree-editor)** - Decision tree visualization editor (TypeScript)
- **[d3-world-map](https://github.com/gaurav101/d3-world-map)** - Interactive world map using D3.js (TypeScript)

### 🦀 Rust Projects
- **[rust-apps](https://github.com/gaurav101/rust-apps)** - Collection of Rust applications
- **[simple-rust-calcuator](https://github.com/gaurav101/simple-rust-calcuator)** - Simple calculator built with Rust
- **[RustroverProjects](https://github.com/gaurav101/RustroverProjects)** - Rust projects collection

### 🌐 Web & API Development
- **[REST-API-ENGINE](https://github.com/gaurav101/REST-API-ENGINE)** - REST API engine in PHP
- **[php-apns](https://github.com/gaurav101/php-apns)** - PHP Apple Push Notification Service
- **[single-file-comment-edior](https://github.com/gaurav101/single-file-comment-edior)** - Comment editor tool
- **[ES6-fundas](https://github.com/gaurav101/ES6-fundas)** - ES6 JavaScript fundamentals

### 📖 Learning & Other Projects
- **[JS-learning](https://github.com/gaurav101/JS-learning)** - JavaScript learning resources
- **[p5](https://github.com/gaurav101/p5)** - p5.js creative coding project
- **[portfolio](https://github.com/gaurav101/portfolio)** - Personal portfolio project
- **[gaurav101.github.io](https://github.com/gaurav101/gaurav101.github.io)** - GitHub Pages personal portfolio

---

## 📊 GitHub Statistics

![GitHub Stats](https://github-readme-stats.vercel.app/api?username=gaurav101&show_icons=true&theme=radical&count_private=true)

![Top Languages](https://github-readme-stats.vercel.app/api/top-langs/?username=gaurav101&layout=compact&theme=radical)

---

## 💡 About Me

- 🎓 Enthusiast in software design patterns and best practices
- 🔍 Exploring Rust and modern programming paradigms
- 📱 Building both frontend and backend solutions
- 🤝 Open to collaboration on interesting projects
- 📈 Continuously learning and improving my craft

---

## 🔗 Connect with Me

- 💼 [LinkedIn](https://linkedin.com/in/gauravkrsingh1)
- 🌍 [Portfolio Website](https://gaurav101.github.io)
- 📧 Reach out for collaboration or to discuss tech!

---

## 📈 What I'm Currently Doing

- 🚀 Exploring Rust for systems programming
- 🎯 Deepening knowledge of Spring Boot and microservices
- 🔄 Working on full-stack React + Java projects
- 🌱 Learning advanced design patterns and architecture

---

*Last Updated: April 2026* ✨
